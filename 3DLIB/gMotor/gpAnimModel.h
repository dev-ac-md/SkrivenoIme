...
/*	Date:	20.11.2002
/*****************************************************************************/
#ifndef __GPANIMMODEL_H__
#define __GPANIMMODEL_H__
...
public:
	...
	bool		isAnimated() const { return (flags & 4) != 0; }
...
public:
	...
	BaseMesh*		getBM( int idx ) { return &(bm[idx]); }
	
}; // ComplexMesh

const int c_StrBufSize		= 2048;
const int c_MaxModelNameLen = 64;
/*****************************************************************************/
/*	Class:	AnimModel	
...
class AnimModel : public ComplexMesh
{
	...
	anmAnimation	anim;
	char			modelName[c_MaxModelNameLen];
...
public:
	...
	bool				LoadTexTable	( FILE* fp, int nBytes );
	bool				Load			( const char* fname );
	void				ProgressAnim	( int frame );
	const char*			GetName			() const;
	
	void				Draw();
	void				DrawAABB( DWORD color );
	void				DrawBoneAABBs( DWORD color );
	void				DrawPlain();
	void				DrawSWSkinning();


	void				SetShader( int shID );
	void				Dump();

	anmSkeleton&		GetSkeleton() { return skeleton; }
	BaseMesh*			GetSubmesh( int idx );

}; // class AnimModel


#endif // __GPANIMMODEL_H__