...
public:
	...
	_inl GPFrameInst( GPSeq* seq,	WORD spr, DWORD col = 0, WORD _lod = 0 );
	_inl GPFrameInst( WORD	 gp,	WORD spr, DWORD col = 0, WORD _lod = 0 ); 
	...
	_inl void		SetNChunks( int nChunks );
...
public:
	...
	_inl DWORD		GetFactor();

protected:
	int					surfTexID;		//  texture memory surface ID
	TexItemLayout		layout;			//  layouter
	GPPixelCacheItem*	nextItem;		//  pointer to the next item in chain
	int					ID;				//  item ID
...
private:
	...
	c2::array<GPPixelCacheItem>		item;
...
public:
	...
	_inl const char*	GetFileName		()						const;
	_inl void			SetFileName( const char* name );

	_inl void			SetGPType		( GPType type );
	_inl GPType			GetGPType		()						const;
...
protected:
	...
	GPType				gpType;			//  sequence type
	String				fileName;

	c2::array<GPFrame>	frame;	
