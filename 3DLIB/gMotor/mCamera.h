...
public:
	...
	void			AddPt( const Vector3D& vPos, const Vector3D& vAt, 
							float dt );

	void			getPosDir( Vector3D& vPos, Vector3D& vAt, float dtime );

	void			InitFromFile( const char* fname );

	void			Reset();
	_inl bool		isEmpty() const;

protected:
private:
	int				numPt;
	Vector3D		pos[c_MaxPathPt];
	Vector3D		at[c_MaxPathPt];

	float			dtime[c_MaxPathPt];
...
public:
	...
	void						RotateAroundLookAtPoint( float ang );

	void						getPickRay( float curX, float curY,
											 Vector3D& orig, Vector3D& dir );

	void						MoveLocal( const Vector3D& mdir, float amount );
