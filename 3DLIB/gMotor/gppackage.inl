...
_inl const char* GPSeq::GetFileName() const
{
	return fileName;
}	
...
_inl GPType	GPSeq::GetGPType() const
{
	return gpType;
}

_inl void GPSeq::SetGPType( GPType type ) 
{
	gpType = type;
}
...
_inl void GPSeq::SetFileName( const char* name )
{
	fileName = name;
}
...
{
	return ((*((DWORD*)this)) >> 12) & 0xFFF;
}
 
_inl int GPChunkHeader::GetY() const
{
	return (*((DWORD*)this)) & 0xFFF;
}
...
/*****************************************************************************/
_inl GPFrameInst::GPFrameInst( GPSeq* seq, WORD spr, DWORD col, WORD _lod )
					: pSeq( seq ), sprID( spr ), color( col ), lod( _lod )
{ 
	gpID	= seq->GetID(); 
	pFrame	= seq->GetFrame( sprID );
}

_inl GPFrameInst::GPFrameInst( WORD gp, WORD spr, DWORD col, WORD _lod )
					: gpID( gp ), sprID( spr ), color( col ), lod( _lod ) 
{
...
_inl void GPFrameInst::SetNChunks( int nChunks )
{ 
	chunk.resize( nChunks ); 
}
...
_inl DWORD GPPixelCacheItem::GetFactor()
{
	int begClient = 0;
	if (client.size() == 0) return 0;
	while (begClient < client.size() && !client[begClient]) begClient++;

	if (begClient >= client.size()) return 0; 
	static const DWORD c_MaximalCacheFactor = 0xFFFFFFFF;
	
	DWORD factor = client[begClient]->GetFactor();
	if (client[begClient]->IsLocked()) return c_MaximalCacheFactor;
	
	for (int i = begClient + 1; i < client.size(); i++)
	{
		if (!client[i]) continue;
		if (client[i]->IsLocked()) return c_MaximalCacheFactor;
		if (client[i]->GetFactor() < factor) factor = client[i]->GetFactor();
	}
	return factor;
} // GPPixelCacheItem::GetFactor
