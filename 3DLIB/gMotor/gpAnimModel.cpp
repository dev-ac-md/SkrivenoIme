...
bool AnimModel::Load( const char* name )
{
	...
	if (!strstr( fileName, ".c2m" ) && !strstr( fileName, ".C2M" ))
	{
		strcat( fileName, ".c2m" );
	}*/
	...
	{
		Log.Message( "Not a .c2m file: %s", fileName );
		return false;
	}
	...
	while (!feof( fp ))
	{
		...
		case btTexTable:		LoadTexTable	( fp, br.blockSize() ); break;
		default:
			Log.Error( "Unknown block type in file %s.", fileName );
			assert( false );
			break;
		}
		br.end();
...
void AnimModel::SetShader( int shID )
{
	for (int i = 0; i < nSub; i++)
	{
		bm[i].setShader( shID );
...
BaseMesh* AnimModel::GetSubmesh( int idx )
{
	assert( idx > 0 && idx < nSub );
	return &(bm[idx]);
}
...
bool AnimModel::LoadSkeleton( FILE* fp, int nBytes )
{
	...
	for (int i = 0; i < skeleton.getNSub(); i++)
	{
		anmSubMesh* sub = skeleton.getSub( i );
		ReadWord( fp, nChunks );
		DWORD flags;
...
bool AnimModel::LoadTexTable( FILE* fp, int nBytes )
{
	int nTex = 0;
	fread( &nTex, 4, 1, fp );
	...
	{
		recodeTex[i] = RC::iRS->GetTextureID( pos );
		pos += strlen( pos ) + 3;
	}
	for (int i = 0; i < nSub; i++)
	{
		...
		bm[i].setTexture( RC::iRS->GetTextureID( "env.jpg" ), 1 );
		bm[i].setShader( RC::iRS->GetShaderID( "smooth" ) );
	}
	return false;
...
/*---------------------------------------------------------------------------*/
void AnimModel::DrawPlain()
{
	Matrix4D wm;
	wm = RC::iRS->GetWorldMatrix();
	Matrix4D matr;
	int nB = skeleton.getNBones();
	for (int i = 0; i < nB; i++)
	{
		...
		matr = bone->transform;
		matr *= wm;
		RC::iRS->SetWorldMatrix( matr );			
		for (int j = 0; j < bone->nChunks; j++)
		{
			const anmMeshChunk& chunk = bone->chunk[j]; 
			anmSubMesh* sub = skeleton.getSub( chunk.subIdx );
			if (sub) RC::iRS->SetTextureMatrix( sub->transform );
			RC::iRS->DrawPrim( bm[chunk.subIdx], 
							chunk.firstPoly, chunk.numPoly, 
							chunk.firstVert, chunk.numVert );   
		}
	}
	RC::iRS->SetWorldMatrix( wm );
} // AnimModel::DrawPlain

void AnimModel::DrawSWSkinning()
{/*
	if (swBM.getNVert() == 0)
	{
		swBM.create( 65535, 0, vfNMP4 );
		swBM.setShader( RC::iRS->GetShaderID( "smooth" ) );
	}
...
void AnimModel::DrawAABB( DWORD color )
{
	AABoundBox aabb;
	skeleton.calcAABB( aabb );
	AuxPrim::DrawAABB( RC::iRS, aabb, color );
} // AnimModel::DrawAABB

void AnimModel::DrawBoneAABBs( DWORD color )
{
	Matrix4D wm;
	wm = RC::iRS->GetWorldMatrix();
	Matrix4D matr;
	...
	for (int i = 0; i < nB; i++)
	{
		...
		matr *= wm;
		RC::iRS->SetWorldMatrix( matr );			
		AuxPrim::DrawAABB( RC::iRS, bone->aabb, color );
	}
} // AnimModel::DrawAABB
...
const char* AnimModel::GetName() const
{
	return modelName;
} // AnimModel::GetName
