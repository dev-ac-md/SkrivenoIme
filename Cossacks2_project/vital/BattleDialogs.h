...
char* GetUnitGPName(word NIndex, int& x, int& y){
	if(NIndex<NATIONS->NMon){
		...
		NewAnimation* AN=NM->Attack;
		if(!AN->Enabled) AN=&NM->Stand;
		if(AN->Enabled){
			NewFrame* NF=AN->Frames;
			x=NF->dx;//+NM->RectDx+NM->RectLx;
			y=NF->dy;//+NM->RectDy+NM->RectLy+(NM->RectLy>>1);
			return (char*)GPS.GPName(AN->Frames->FileID);
		}		
	}
	return NULL;
}
...
int GetAttackNFrame(word NIndex, word AttIndex){
	if(NIndex<NATIONS->NMon&&AttIndex<NAttTypes){
		NewAnimation* AN=NATIONS->Mon[NIndex]->newMons->Attack+AttIndex;
		if(AN->Enabled){
			return AN->NFrames;
		}
	}
...
CEXPORT
char* GetPlayerName(byte NI){	
	if(NI>=7) return NULL;
		else return PINFO[NI].name;
}

CEXPORT
byte GetPlayerColor(word ID){
	if(ID>7) return 255;
		else return NatRefTBL[PINFO[ID].ColorID];
}

CEXPORT
byte GetMyPlayerColor(){
	return NatRefTBL[GetMyNation()];
}

CEXPORT
byte GetMyPlayerID(){
	byte mn=GetMyNation();
	for(int i=0;i<8;i++)if(PINFO[i].ColorID==mn) return i;
	return 255;
}
...
char* GetCurMissionMapName(){
	if(CurrentMap[0]) return CurrentMap;
	/*
