...
int MotionField::CheckHLine(int x,int y,int Lx){
	...
	/*
	int MAPH=int(MapH);
	if(x>0&&y>0&&x+Lx-1<MAPSX&&y<MAPSY&&Lx<=24){
		switch(ADDSH){
		case 1:
			__asm{
				mov		eax,x
...
void CreateFullPath(int x1,int y1,OneObject* OB){
	...
	while(OB->NIPoints){
		int xx1=OB->PathX[OB->NIPoints-1];
		int yy1=OB->PathY[OB->NIPoints-1];
		if(Norma(OB->x-xx1,OB->y-yy1)<3)OB->NIPoints--;
		else break;
		if(!OB->NIPoints){
			free(OB->PathX);
			free(OB->PathY);
	...
	if(AllowPathDelay)OB->PathDelay=(rando()&7);
};
//void addrand(int i);
#define sssh 1
extern int COUNTER;
bool OneObject::CreatePrePath(int x1,int y1){
	...
	addrand(y1);
	AllowPathDelay=false;
	if(LockType==1||newMons->MotionStyle==3){
		return CreatePrePath2(x1,y1);
	};
	int x=this->x;
	int y=this->y;
	FindBestPosition(this,&x,&y,40);
...
loopsEnd:
	...
	if(Rtx!=x1||Rty!=y1){
		...
		if(Rpp<MaxP-9){
			...
			PathX=znew(short,NIPoints+addNIPoints);
			PathY=znew(short,NIPoints+addNIPoints);
			if(OldPathX){
...
void OneObject::CreatePath(int x1,int y1){
	...
	MotionField* MFI=&MFIELDS[LockType];
	int D=(newMons->StopDistance>>8)+1;
	if(CPdestX==x1&&CPdestY==y1){
		if(PathX){
			if(NIPoints){
				...
				if(CObjIndex!=0xFFFF){
					if(MFI->CheckBar(x,y,Lx,Lx)&&rando()<8192){
						NIPoints=0;
						free(PathX);
						free(PathY);
						PathX=NULL;
					...
					if(NIPoints>=2){
						int cx=PathX[NIPoints-2];
						int cy=PathY[NIPoints-2];
						if(CheckDirectConnectionToDestPoint(x,y,cx,cy))D=10000;
					}else if(NIPoints==1&&rando()<2048){
						int cx=PathX[0];
						int cy=PathY[0];
						if(CheckDirectConnectionToDestPoint(x,y,cx,cy))D=10000;
					};
			...
			if(MFI->CheckBar(x,y,Lx,Lx)&&rando()<16384){
				NIPoints=0;
				addrand(52);
				CreateFullPath(x1,y1,this);
			}else{
