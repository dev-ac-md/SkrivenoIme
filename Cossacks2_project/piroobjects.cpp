...
void PiroSystem::Process(){
	...
	for(int i=0;i<NPObj;i++){
		...
		PO->vx=PO->vx0*az;
		PO->vy=PO->vy0*az;
		PO->vz=PO->vz0*az*exp(-PO->dz/90/C2);
		PO->vx0*=0.99;
		PO->vy0*=0.99;
		PO->vz0*=0.99;
		PO->LifeTime++;
...
void AddSmokeOnHorseStep(OneObject* OB){	
	...
	if(OB&&!OB->Sdoxlo){
		int x=OB->RealX>>4;
		int y=OB->RealY>>4;
		int MR=20+PS.NPObj/2;
		if(PS.GetDistToNearest(x,y)>MR&&PS.NPObj<1000){
			int tofs=(x>>5)+(y>>5)*VertInLine;
			if(tofs>0&&tofs<MaxPointIndex){
				int T=TexMap[tofs];
				PS.AddPiroObject(x,y,OB->RZ,0,0,3.5,0.01,0.01+float(rand()&255)/12000,-0.05+float(rand()&255)*0.025/255,70,TXCOLOR[T]+0x00303030);
			};
		};
	}
}
