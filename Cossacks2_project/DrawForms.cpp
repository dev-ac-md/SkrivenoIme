...
void PushWindow(TempWindow* W){
	...
	W->WindLy	= vp.height;
	W->WindX	= vp.x;
	W->WindX1	= vp.x + vp.width - 1;
	W->WindY	= vp.y;
	W->WindY1	= vp.y + vp.height - 1;
#endif // _USE3D
};
...
void DrawRect4(int x0,int y0,int x1,int y1,Corners* CR,int GP_File){
	...
	if(LEFTLY<64)LEFTLY=64;
	int RIGHTLY=GPS.GetGPHeight(GP_File,CR->LR);
	if(RIGHTLY<64)RIGHTLY=64;

	//horisontal lines
	IntersectWindows(x0+LX2,y0-LLY2,x1-LX2-1,y1+LLY2);
	int N=(x1-x0+1-ULLX)/UPLX;
	for(int i=0;i<=N+2;i++){
		if(CR->LU!=-1)GPS.ShowGPLayers(x0+i*UPLX+LX2,y0-LLY2,GP_File,CR->LU,MyNation,GP_OPT);
	};
	PopWindow(&TEMP);
	IntersectWindows(x0+LX3,y0-LLY2,x1-LX3-1,y1+LLY2);
	N=(x1-x0+1-ULLX)/DNLX;
	for(int i=0;i<=N+2;i++){
		if(CR->LD!=-1)GPS.ShowGPLayers(x0+i*DNLX+LX3,y1-LX3,GP_File,CR->LD,MyNation,GP_OPT);
	};
	//vertical lines
	PopWindow(&TEMP);
	...
	for(int i=0;i<=N+1;i++){
		if(CR->LL!=-1)GPS.ShowGPLayers(x0-LX3,y0+i*LEFTLY+LLY2,GP_File,CR->LL,MyNation,GP_OPT);
	};
	N=(y1-y0+1-LSLY)/RIGHTLY;
	for(int i=0;i<=N+1;i++){
		if(CR->LR!=-1)GPS.ShowGPLayers(x1-LX3,y0+i*RIGHTLY+LLY2,GP_File,CR->LR,MyNation,GP_OPT);
	};
	//corners
	PopWindow(&TEMP);
	IntersectWindows(x0-LX2,y0-LLY2,(x1+x0)/2-1,(y1+y0)/2-1);
	if(CR->CLU!=-1)GPS.ShowGPLayers(x0-LX2,y0-LLY2,GP_File,CR->CLU,MyNation,GP_OPT);

	PopWindow(&TEMP);
	IntersectWindows((x0+x1)/2,y0-LLY2,x1+LX2,(y1+y0)/2-1);
	if(CR->CRU!=-1)GPS.ShowGPLayers(x1-LX2,y0-LLY2,GP_File,CR->CRU,MyNation,GP_OPT);

	PopWindow(&TEMP);
	IntersectWindows(x0-LX3,(y1+y0)/2,(x0+x1)/2-1,y1+LLY2);
	if(CR->CLD!=-1)GPS.ShowGPLayers(x0-LX3,y1-LLY2,GP_File,CR->CLD,MyNation,GP_OPT);

	PopWindow(&TEMP);
	IntersectWindows((x0+x1)/2,(y1+y0)/2,x1+LX3,y1+LLY2);
	if(CR->CRD!=-1)GPS.ShowGPLayers(x1-LX3,y1-LLY2,GP_File,CR->CRD,MyNation,GP_OPT);
	PopWindow(&TEMP);
};
