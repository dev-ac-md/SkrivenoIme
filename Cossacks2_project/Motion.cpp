...
void MotionHandlerOfNewSheeps(OneObject* OB){
	...
	}else OB->NewCurSprite=0;
	{
		...
		bool MRes=CalculateMotion2Discret(OB);
		bool NeedR=OB->RealDir!=OLDDIR;
		if(Inprocess)OB->RealDir=OLDDIR;
CalcMotion:
		if(OB->Speed){
			...
			if(AllowGMotion&&AllowLMotion){
				...
				MoveObject(OB);
				if((NeedR&&!Inprocess)||!OB->Speed){
					OB->NewAnm=&NMN->Stand;//PStand[0];
					OB->NewCurSprite=0;
					sp=0;
				};
				OB->NewCurSprite=sp;
				if(OB->NewCurSprite>=OB->NewAnm->NFrames)OB->NewCurSprite=0;
				OB->GLock=false;
			...
			int NR=NMN->MotionL.Rotations;
			if(OB->NewCurSprite>=OB->NewAnm->NFrames-FrmDec){
				void AttackObjLink(OneObject* OBJ);
				if(OB->LocalOrder&&OB->LocalOrder->DoLink==&AttackObjLink&&OB->NewState){
					OB->NewAnm=&NMN->PStand[OB->NewState-1];
					OB->NewCurSprite=0;
				}else{
					OB->NewAnm=&NMN->Stand;
					OB->NewCurSprite=0;
