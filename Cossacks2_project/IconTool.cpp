...
void OneIcon::Draw(int x,int y){
	...
	if(Selected){
		if(!CYELLOW)CYELLOW=GetPaletteColor(255,200,64);
		Xbar(x-1,y-1,IconLx+1+2,IconLy+1+2,CYELLOW);
	};
    if((Type&2)&&MaxHealth>0&&Health>0){//Health bar
    ...
    if((Type&4)&&MaxStage>0&&Stage>0){//Stage bar
